 ...
 * 
 */
package com.galaxy.constants.validate;
...
import com.galaxy.constants.convertLine.ConversationLine;
import com.galaxy.constants.error.ErrorCodes;
import com.galaxy.constants.error.ErrorMessage;
import com.galaxy.service.impl.RomanNumbersServiceImpl;
 ...
 * 
 */
public class GalaxyValidate {

	private Scanner scan;
	private ConversationLine conversationLine;
	private ErrorMessage eMessage;
	private RomanNumbersServiceImpl romanNumbersServiceImpl;
	/**
	...
	public GalaxyValidate() {
		...
		this.romanNumbersServiceImpl = new RomanNumbersServiceImpl();
	}
	 ...
	 * <b>Ex:</b> Pradeep is 78 , error message : <i>Input format is wrong ! input
	 * discarded</i>
	 * </p>
	 * 
	...
	private void processAssignmentLine(String line) {
		...
		// The first part i.e splited[0] is identifier and second part is i.e splited[2]
		// is the value
	...
	private void processHowMuchQuestion(String line) {
		try {
			...
			// Now since the string will contain only identifiers,break them into words by
			...
			if (!errorOccured) {
				...
				romanResult = romanNumbersServiceImpl.romanToArabic(romanResult);
	...
	private void processCreditsLine(String line) {
		try {
			...
			int romanNumber = Integer.parseInt(romanNumbersServiceImpl.romanToArabic(roman));
	...
	private void processHowManyCreditsQuestion(String line) {
		...
		try {
			...
			if (found) {
				...
				if (roman.length() > 0) {
					finalres = (int) (Integer.parseInt(romanNumbersServiceImpl.romanToArabic(roman)) * res);
				}
